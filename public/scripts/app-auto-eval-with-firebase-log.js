(function(document) {
  ...
  app.submLog = null; // Firebase node
  ...
  app._onLogin = function() {
    ...
    // prepare firebase logger
    var userHash = app.user.email.split('@')[0].replace(/[^\w]/g, '_');
    var remoteUser = app.firebaseDB.ref('/submission-logs/' + userHash);
    var upd = {
      _uid: app.user.id,
      _email: app.user.email,
    };
    remoteUser.update(upd, function(err) {
      if (err) {
        console.error('onLogin:firebase -> err:', err);
      } else {
        console.log('onLogin:firebase -> OK');
        app.submLog = remoteUser.child('log');
      }
    });
  };
  ...
  app.logStudentSubmission = function(subm) {
    var newStudentSubmission = app.submLog.push();
    newStudentSubmission.set(Object.assign({
